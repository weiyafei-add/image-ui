...
export function TabBar() {
  ...
  return (
    <div className="flex">
      <ScrollButton direction="left" isVisible={!isAtStart} onClick={scrollToStart} />
      <div
      ...
      <ScrollButton direction="right" isVisible={!isAtEnd} onClick={scrollToEnd} />
    </div>
  );
}
...
function ScrollButton({
  ...
  return (
    <div className="bg-panel-accent w-12 h-full flex items-center justify-center">
      <Button size="icon" className="rounded-[0.4rem] w-4 h-7 bg-background/10!" onClick={onClick}>
        <Icon />
      </Button>
    </div>
  );
